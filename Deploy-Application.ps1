...
Try {
	...
	[string]$appVersion = '8.4.4'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.2'
	[string]$appScriptDate = '2022.08.24'
	...
	$64bitAppInstallName = "Notepad++"
	# Application install Path.
	$64bitAppInstallPath = "$PSScriptRoot\Files\npp.8.4.4.Installer.x64.exe"
	# Application  install parameters.
	$64bitAppInstallParam = "/S -WindowStyle Hidden" 
	...
	#$RemoveAppNamesMSI = @("")
	$RemoveAppNamesEXE = @("Notepad++")

	##* Application uninstall parameters.
	# 64-bit application
	# 64-bit application uninstall name.
	$64bitAppUninstallName = "Notepad++"
	# 64-bit application uninstall path.
	$64bitAppUninstallPath = "$Env:ProgramFiles\Notepad++\uninstall.exe"
	# 32-bit application uninstall path.
	$32bitAppUninstallPath = "C:\Program Files (x86)\Notepad++\uninstall.exe"
	# Application uninstall parameters.
	$64bitAppUninstallParam = "/S -WindowStyle Hidden"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		# If 32-bit application is installed, uninstall it.
		# Else, uninstall 64-bit application.
		...
		Foreach ($RemoveAppNameEXE in $RemoveAppNamesEXE) {
			...
			If ($null -ne $RemoveAppNamesEXECheck) {
				...
				# Uninstall previous 64-bit EXE versions of application(s).      
				Try {
			...
			Else {
				# Uninstall previous 32-bit EXE versions of application(s).      
				Try {
					Execute-Process -FilePath "$32bitAppUninstallPath" -Arguments "$64bitAppUninstallParam"
					Remove-Folder -Path "C:\Program Files (x86)\Notepad++"
				}
				# If uninstall failed, log results. Exit script.
				Catch [System.Exception] {
					Write-Log -Message "Uninstalling previous EXE versions of $RemoveAppNameEXE failed with error: $_."
		...
		# Install add-on.
		# This resolves an error that occurs on some PCs.
		# If installation failed, log results. Exit script. 
		Try {
			Write-log -Message "Installing right click add-on."
			regsvr32 /s /i /NppShell_06.dll
			Write-Log -Message "Installation complete." 
		}
		# If install failed, log results. Exit script.
		Catch [System.Exception] {
			Write-Log -Message "Installation failed with error: $_."
			Write-Log -Message "Exiting script with error."
			Exit-Script -ExitCode 1627
		}
